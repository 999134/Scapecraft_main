	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("The Kalaphite Queen has dropped a dragon chainbody!")));
		}
		if (quickvar1 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.dlong), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("The Kalaphite Queen has dropped a dragon longsword!")));
		}
		if (quickvar2 <= 1) {
		...
		if (quickvar3 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("The Kalaphite Queen has dropped a crystal bow!")));
		}
		if (quickvar4 <= 1) {
			entityDropItem(new ItemStack(mod_phat.mithLeggings), 1);
		}
		if (quickvar5 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.D2H), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The Kalaphite Queen has dropped a dragon 2H!")));
		}
		...
		if (XP <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The Kalaphite Queen has been slain!")));
		}
	}
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
		...
		if (!worldObj.isRemote) {
			...
			for (int k = 0; k < j; ++k) {
				...
				entityKQ2.setLocationAndAngles(this.posX + (double) f, this.posY + 0.5D, this.posZ + (double) f1,
