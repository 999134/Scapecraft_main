	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar1 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor dropped a crystal bow!")));
		}
		if (quickvar2 <= 1) {
			entityDropItem(new ItemStack(mod_phat.dragonHelmet), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor dropped a dragon helmet!")));
		}
		if (quickvar3 <= 2) {
			entityDropItem(new ItemStack(mod_MagicTree.Magicfruit), 1);
		}
		if (quickvar4 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.BandosHilt), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor dropped a bandos hilt!")));
		}
		if (quickvar5 <= 1) {
		...
		if (quickvar8 <= 1) {
			entityDropItem(new ItemStack(mod_phat.runegHelmet), 1);
		}
		if (quickvar9 <= 1) {
			entityDropItem(new ItemStack(mod_phat.runegLeggings), 1);
		}
		if (quickvar10 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.MithoreSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor dropped a mith ore spawner!")));
		}
		if (quickvar11 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.AddyoreSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor dropped an addy ore spawner!")));
		}
		if (quickvar12 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.RuneoreSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor dropped a rune ore spawner!")));
		}
		if (quickvar13 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.CoalSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor dropped a coal ore spawner!")));
		}
		if (quickvar14 <= 1) {
			entityDropItem(new ItemStack(mod_phat.chest), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor dropped a bandos chestplate!")));
		}
		if (quickvar15 <= 1) {
			entityDropItem(new ItemStack(mod_phat.pphatLeggings), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor dropped some bandos tassets!")));
		}
		// if(quickvar15 <= 1){entityDropItem(new ItemStack(mod_phat.legs), 1);}

		int XP = rand.nextInt(1) + 1;
		if (XP <= 1) {
			entityDropItem(new ItemStack(mod_Flower.BXPORB), 2);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("General Graardor was slain!")));
		}
	}
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
		...
		if (!worldObj.isRemote) {
			ModLoader.getMinecraftInstance().thePlayer
					.addChatMessage("\u00a7CGeneral Graardor: Split their skulls! Break their bones!");
		}
		if (!worldObj.isRemote) {
