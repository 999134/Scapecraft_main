...
public class EntityHighMage extends EntityMob implements IRangedAttackMob {
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
		...
		if (!worldObj.isRemote) {
			...
			for (int k = 0; k < j; ++k) {
				...
				entityBlackKnight.setLocationAndAngles(this.posX + (double) f, this.posY + 0.5D,
						this.posZ + (double) f1, this.rand.nextFloat() * 360.0F, 0.0F);
	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar <= 1) {
			...
			if (quickvar1 <= 1) {
				entityDropItem(new ItemStack(mod_MagicBow.DarkBow), 1);
				MinecraftServer.getServer().getConfigurationManager()
						.sendChatMsg((new ChatMessageComponent().addText("The High Mage has dropped a dark bow")));
			}
		}
		if (quickvar1 <= 1) {
		...
		if (quickvar2 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The High Mage has dropped a dragon long")));
		}

		if (quickvar3 <= 1) {
			entityDropItem(new ItemStack(Item.ingotGold), 4);
		}
		if (quickvar4 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.MithoreSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The High Mage has dropped a mith ore spawner")));
		}
		if (quickvar5 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.AddyoreSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The High Mage has dropped an addy ore spawner")));
		}

		if (quickvar7 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.CoalSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The High Mage has dropped a coal spawner")));
		}
		if (quickvar8 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.PickaxeGaloreg), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The High Mage has dropped a dragon pickaxe")));
		}
		if (quickvar9 <= 1) {
			entityDropItem(new ItemStack(Item.appleGold), 1);
		}
		...
		if (quickvar6 > 15 && quickvar6 <= 20) {
			entityDropItem(new ItemStack(mod_Flower.Vtab), 30);
		}
		if (quickvar6 > 20 && quickvar6 <= 25) {
			entityDropItem(new ItemStack(mod_Flower.Htab), 30);
		}
		...
		if (XP <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The High Mage has been defeated!")));
		}
		...
		if (spawnvar >= 300) {
			if (!worldObj.isRemote) {
				...
				for (int k = 0; k < j; ++k) {
					...
					entityEliteBlackKnight.setLocationAndAngles(this.posX + (double) f, this.posY + 0.5D + 1,
							this.posZ + (double) f1 + 4, this.rand.nextFloat() * 360.0F, 0.0F);
