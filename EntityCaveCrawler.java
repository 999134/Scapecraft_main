...
protected String getLivingSound()
    ...
    protected void dropFewItems(boolean par1, int par2){
        ...
        if(quickvar1 <= 1){
        	...
        	MinecraftServer.getServer().getConfigurationManager().sendChatMsg((new ChatMessageComponent().addText("A cave crawler dropped a crystal bow!")));
        }
if(quickvar2 <= 1){
...
if(quickvar4 <= 1){
	...
	if(quickvar6 <= 1){entityDropItem(new ItemStack(mod_BlocksGalore.DD), 1);
  	MinecraftServer.getServer().getConfigurationManager().sendChatMsg((new ChatMessageComponent().addText("A cave crawler dropped a dragon dagger!")));}

	if(quickvar7<= 1){entityDropItem(new ItemStack(Item.coal,3), 1);}
