	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar1 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("A Tormented Demon has dropped a crystal bow!")));
		}
		if (quickvar2 <= 1) {
			entityDropItem(new ItemStack(mod_phat.dragonHelmet), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped a dragon helmet!")));
		}
		if (quickvar3 <= 2) {
		...
		if (quickvar4 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped a dragon battleaxe!")));
		}
		if (quickvar5 <= 1) {
			entityDropItem(new ItemStack(mod_phat.mithChestplate), 1);
		}
		if (quickvar6 <= 1) {
			entityDropItem(new ItemStack(mod_phat.dragonlChestplate), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped a lava dragon chainbody!")));
		}
		if (quickvar7 <= 1) {
			entityDropItem(new ItemStack(mod_phat.dragonlBoots), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped a lava dragon boots!")));
		}
		if (quickvar8 <= 1) {
			entityDropItem(new ItemStack(mod_phat.dragonlHelmet), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped a lava dragon helmet!")));
		}
		if (quickvar9 <= 1) {
			entityDropItem(new ItemStack(mod_phat.dragonlLeggings), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped a lava dragon platelegs!")));
		}
		if (quickvar10 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.MithoreSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped a mith ore spawner!")));
		}
		if (quickvar11 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.AddyoreSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped an Addy ore spawner!")));
		}
		if (quickvar12 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.RuneoreSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped a rune ore spawner!")));
		}
		if (quickvar13 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.CoalSpawn), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Tormented Demon has dropped a coal ore spawner!")));
		}
		...
		if (XP <= 1) {
			entityDropItem(new ItemStack(mod_Flower.BXPORB), 1);
			// MinecraftServer.getServer().getConfigurationManager().sendChatMsg((new
			// ChatMessageComponent().addText("A Tormented Demon has been defeated!")));}
		}
	}
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
		...
		if (!worldObj.isRemote) {
			...
			for (int k = 0; k < j; ++k) {
				...
				entityLavaBlock.setLocationAndAngles(this.posX + (double) f, this.posY + 0.5D + 1,
						this.posZ + (double) f1 + 4, this.rand.nextFloat() * 360.0F, 0.0F);
				this.worldObj.spawnEntityInWorld(entityLavaBlock);
				this.worldObj.spawnEntityInWorld(entityLavaBlock);
				this.worldObj.spawnEntityInWorld(entityLavaBlock);

				entityBlaze.setLocationAndAngles(this.posX + (double) f, this.posY + 0.5D, this.posZ + (double) f1,
