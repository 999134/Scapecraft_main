...
public class EntityDoctor extends EntityMob
{
...
protected String getLivingSound()
    ...
    public boolean interact(EntityPlayer par1EntityPlayer)
    {
    	...
    	if (itemstack != null && itemstack.itemID == mod_Flower.beer.itemID && !par1EntityPlayer.capabilities.isCreativeMode)
        {
            if (itemstack.stackSize-- == 1)
            {
                par1EntityPlayer.inventory.setInventorySlotContents(par1EntityPlayer.inventory.currentItem, new ItemStack(mod_BlocksGalore.Stake));
                if(!worldObj.isRemote) {  par1EntityPlayer.addChatMessage("\u00a7EDr Harlow: Take this, it should allow you to kill the vampire, but you will still need your best armor");}
            	
            }
            else if (!par1EntityPlayer.inventory.addItemStackToInventory(new ItemStack(mod_BlocksGalore.Stake)))
            {
                par1EntityPlayer.dropPlayerItem(new ItemStack(mod_BlocksGalore.Stake.itemID, 1, 0));
            }
    		...
    		if(!worldObj.isRemote) {par1EntityPlayer.addChatMessage("\u00a7EDr Harlow: Bring me a beer and then we can talk");}
