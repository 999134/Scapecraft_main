	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
		...
		if (!worldObj.isRemote) {
			...
			for (int k = 0; k < j; ++k) {
				...
				entityKingsGuard.setLocationAndAngles(this.posX + (double) f, this.posY + 0.5D, this.posZ + (double) f1,
	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar1 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The king has dropped a saradomin sword!")));
		}
		if (quickvar2 <= 1) {
		...
		if (quickvar3 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The King has dropped a dragon dagger")));
		}
		if (quickvar4 <= 1) {
		...
		if (quickvar6 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("The King has been killed, varrock panics")));
		}
	}
		...
		if (!worldObj.isRemote) {
			if (chat == 1) {
				ModLoader.getMinecraftInstance().thePlayer
						.addChatMessage("\u00a7EKing: Show better respect for your monarch");
			}
			if (chat == 2) {
				ModLoader.getMinecraftInstance().thePlayer
						.addChatMessage("\u00a7EKing: *The king ignores you completely");
			}
			if (chat == 3) {
				ModLoader.getMinecraftInstance().thePlayer
						.addChatMessage("\u00a7EKing: You look like you could be of service to the realm");
			}
			if (chat == 4) {
				ModLoader.getMinecraftInstance().thePlayer.addChatMessage(
						"\u00a7EKing: My crown? No of course it isn't for sale, your neck should come off just for asking");
			}
			if (chat == 5) {
				ModLoader.getMinecraftInstance().thePlayer
						.addChatMessage("\u00a7EKing: Stay vigilant against the black knights");
			}
			if (chat == 6) {
				ModLoader.getMinecraftInstance().thePlayer.addChatMessage("\u00a7EKing: Its good to be king");
			}
			if (chat == 7) {
				ModLoader.getMinecraftInstance().thePlayer
						.addChatMessage("\u00a7EKing: I used be to quite the top quality fighter back in my day");
			}
			if (chat == 8) {
				ModLoader.getMinecraftInstance().thePlayer
						.addChatMessage("\u00a7EKing: What do you want? Looking to contribute extra taxes?");
			}
			if (chat == 9) {
				ModLoader.getMinecraftInstance().thePlayer
						.addChatMessage("\u00a7EKing: We are not recruiting right now");
			}
			if (chat == 10) {
				ModLoader.getMinecraftInstance().thePlayer.addChatMessage("\u00a7EKing: Go bother someone else");
			}
		}
