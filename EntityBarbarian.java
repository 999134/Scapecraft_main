	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar1 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("A Barbarian dropped some Dragon Platelegs!")));
		}
		if (quickvar2 <= 1) {
		...
		if (quickvar7 <= 1) {
			entityDropItem(new ItemStack(mod_phat.helmet), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("A Barbarian dropped a Berserker helm!")));
		}
