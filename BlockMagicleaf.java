...
public class BlockMagicleaf extends BlockLeavesBase
{
        private int baseIndexInPNG;
        int adjacentTreeBlocks[];
        
        protected BlockMagicleaf(int i, int j)
        {
                        ...
                        for(int j1 = -l; j1 <= l; j1++)
                        {
                                for(int k1 = -l; k1 <= l; k1++)
                                {
                                        for(int l1 = -l; l1 <= l; l1++)
                                        {
                                                ...
                                                if(i2 == mod_MagicTree.Magicsapling.blockID)
                                                {
        ...
        public void dropBlockAsItemWithChance(World par1World, int par2, int par3, int par4, int par5, float par6, int par7)
        {
            if (!par1World.isRemote)
            {
                ...
                {
                    this.dropBlockAsItem_do(par1World, par2, par3, par4, new ItemStack(mod_MagicTree.Magicfruit, 1, 0));
                }
            }
        }
                                ...
                                for(int l1 = -byte0; l1 <= byte0; l1++)
                                {
                                        for(int k2 = -byte0; k2 <= byte0; k2++)
                                        {
                                                for(int i3 = -byte0; i3 <= byte0; i3++)
                                                {
                                                        ...
                                                        if(k3 == mod_MagicTree.Magiclog.blockID)
                                                        {
                                                                adjacentTreeBlocks[(l1 + k1) * j1 + (k2 + k1) * byte1 + (i3 + k1)] = 0;
                                                                continue;
                                                        }
                                                        if(k3 == mod_MagicTree.Magicleaf.blockID)
                                                        {
        ...
        public void registerIcons(IconRegister ir)
        {
                ...
                {
                this.blockIcon = ir.registerIcon("magicleaf");
                }
        }
