...
public class EntityCook extends EntityMob
{
...
protected String getLivingSound()
    ...
    public boolean interact(EntityPlayer par1EntityPlayer)
    {
    	...
    	if (itemstack != null && itemstack.itemID == Item.cake.itemID && !par1EntityPlayer.capabilities.isCreativeMode)
        {
            if (itemstack.stackSize-- == 1)
            {
                par1EntityPlayer.inventory.setInventorySlotContents(par1EntityPlayer.inventory.currentItem, new ItemStack(mod_Flower.QuestPoint2));
                if(!worldObj.isRemote) { par1EntityPlayer.addChatMessage("\u00a7ELumbridge Cook: Thank you, I don't actually have any reward for you, but I never promised one, next time be more careful with who you help");
                }
            }
            else if (!par1EntityPlayer.inventory.addItemStackToInventory(new ItemStack(mod_Flower.QuestPoint2)))
            {
                par1EntityPlayer.dropPlayerItem(new ItemStack(mod_Flower.QuestPoint2.itemID, 1, 0));
            }
            return true;
        }
    	   	   	
    	else
    		if(!worldObj.isRemote) {par1EntityPlayer.addChatMessage("\u00a7ELumbridge Cook: Please bring me a cake, the Duke's Birthday is soon and I can't work out how to make one on minecraft, who uses raw wheat in cooking?");
    		}
