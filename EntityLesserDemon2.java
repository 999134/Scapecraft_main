	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("A Lesser Demon has dropped a chaotic rapier!")));
		}
		if (quickvar1 <= 1) {

			entityDropItem(new ItemStack(mod_BlocksGalore.DBA), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Lesser Demon has dropped a dragon battleaxe!")));
		}
		if (quickvar2 <= 1) {
		...
		if (quickvar5 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A Lesser Demon has dropped a godsword shard 3!")));
		}
