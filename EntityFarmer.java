...
public class EntityFarmer extends EntityMob {
	...
	public boolean attackEntityAsMob(Entity par1Entity) {
		if (super.attackEntityAsMob(par1Entity)) {
			if (par1Entity instanceof EntityBlackKnight || par1Entity instanceof EntityGoblin
					|| par1Entity instanceof EntityTheif) {
	...
	public boolean interact(EntityPlayer par1EntityPlayer) {
		...
		if (!worldObj.isRemote) {
			if (props.getTheifcd() >= 99) {
				if (!worldObj.isRemote) {
					ModLoader.getMinecraftInstance().thePlayer.addChatMessage("\u00a7EFarmer: Get off my land");
				}
			}
		}
