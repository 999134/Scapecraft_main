...
public class EntityCaveCrawler2Boss extends EntityMob {
	private float moveSpeed;

	public EntityCaveCrawler2Boss(World par1World) {
		super(par1World);

		this.setSize(this.width * 2.5F, this.height * 1.5F);
		this.moveSpeed = 0.5F;
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setAttribute(220.0D);
		// Follow Range - default 32.0D - min 0.0D - max 2048.0D
		this.getEntityAttribute(SharedMonsterAttributes.followRange).setAttribute(32.0D);
		// Movement Speed - default 0.699D - min 0.0D - max Double.MAX_VALUE
		this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setAttribute(0.850D);
		// Attack Damage - default 2.0D - min 0.0D - max Doubt.MAX_VALUE
		this.getEntityAttribute(SharedMonsterAttributes.attackDamage).setAttribute(6.0D);
		this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance).setAttribute(0.6D);
	}

	public String getEntityName() {
		return "Cave Crawler Boss";
	}
	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar <= 1) {
			entityDropItem(new ItemStack(mod_phat.pphatBoots), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("The overgrown cave crawler dropped some climbing boots!")));
		}
		if (quickvar1 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.dlong), 1);
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("The overgrown cave crawler dropped a dragon longsword!")));
		}
		if (quickvar3 <= 1) {
			entityDropItem(new ItemStack(mod_phat.blackChestplate), 1);
		}
		if (quickvar4 <= 1) {
			entityDropItem(new ItemStack(mod_phat.mithLeggings), 1);
		}
		if (quickvar5 <= 1) {
		...
		// Medium value table//
		if (quickvar6 > 5 && quickvar6 <= 10) {
			entityDropItem(new ItemStack(Item.ingotGold), 1);
		}
		if (quickvar6 > 10 && quickvar6 <= 15) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Mithore), 1);
		}
		if (quickvar6 > 15 && quickvar6 <= 20) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Whitesword), 1);
		}
		if (quickvar6 > 20 && quickvar6 <= 25) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Blacksword), 1);
		}
		if (quickvar6 > 25 && quickvar6 <= 30) {
			entityDropItem(new ItemStack(mod_phat.blackLeggings), 1);
		}
		if (quickvar6 > 30 && quickvar6 <= 35) {
			entityDropItem(new ItemStack(mod_phat.blackBoots), 1);
		}
		if (quickvar6 > 35 && quickvar6 <= 50) {
			entityDropItem(new ItemStack(Item.coal, 3), 1);
		}
		if (quickvar6 > 50 && quickvar6 <= 55) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Mithore), 1);
		}
		if (quickvar6 > 50 && quickvar6 <= 55) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Addyore), 1);
		}
		if (quickvar6 > 55 && quickvar6 <= 75) {
			entityDropItem(new ItemStack(Item.arrow, 10), 1);
		}
		if (quickvar6 > 75 && quickvar6 <= 85) {
			entityDropItem(new ItemStack(Item.wheat, 10), 1);
		}
		if (quickvar6 > 85 && quickvar6 <= 90) {
			entityDropItem(new ItemStack(mod_Flower.antifire), 1);
		}
		if (quickvar6 > 85 && quickvar6 <= 90) {
			entityDropItem(new ItemStack(mod_Flower.sarabrew), 1);
		}
		if (quickvar6 > 85 && quickvar6 <= 90) {
			entityDropItem(new ItemStack(mod_Flower.superstr), 1);
		}
		if (quickvar6 > 85 && quickvar6 <= 90) {
			entityDropItem(new ItemStack(mod_Flower.superdef), 1);
		}
		if (quickvar6 > 90 && quickvar6 <= 91) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Runeaxe), 1);
		}
		if (quickvar6 > 91 && quickvar6 <= 92) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Runesword), 1);
		}
		if (quickvar6 > 92 && quickvar6 <= 93) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Runeshovel), 1);
		}
		if (quickvar6 > 93 && quickvar6 <= 94) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Runepickaxe), 1);
		}
		if (quickvar6 > 94 && quickvar6 <= 95) {
			entityDropItem(new ItemStack(mod_BlocksGalore.Runelump), 1);
		}

		int XP = rand.nextInt(1) + 1;
		if (XP <= 1) {
			entityDropItem(new ItemStack(mod_Flower.BXPORB), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("the overgrown cave crawler was slain!")));
		}
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
		...
		if (!worldObj.isRemote) {
			ModLoader.getMinecraftInstance().thePlayer.addChatMessage("\u00a7COvergrown Cave Crawler: HISS!");
		}
		if (!worldObj.isRemote) {
			...
			if (movement <= 20 && movement >= 10) {
				this.motionZ = 2.8D;
			}
			if (movement <= 30 && movement >= 20) {
				this.motionZ = -2.4D;
			}
			if (movement <= 40 && movement >= 30) {
				this.motionZ = -2.8D;
			}
			if (movement <= 50 && movement >= 40) {
				this.motionX = 2.4D;
			}
			if (movement <= 60 && movement >= 50) {
				this.motionX = 2.8D;
			}
			if (movement <= 70 && movement >= 60) {
				this.motionX = -3.4D;
			}
			if (movement <= 80 && movement >= 70) {
				this.motionX = -1.4D;
			}
			if (movement <= 90 && movement >= 80) {
				this.motionZ = 2.4D;
				this.motionX = -2.0D;
			}
			if (movement <= 100 && movement >= 90) {
				this.motionZ = -2.4D;
				this.motionX = 2.0D;
			}
			;
