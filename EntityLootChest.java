...
public class EntityLootChest extends EntityMob {
	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar2 <= 1) {
			entityDropItem(new ItemStack(mod_Flower.fishpieuncooked), 1);
		}
		if (quickvar2 <= 1) {
			entityDropItem(new ItemStack(mod_Flower.fishpieuncooked), 1);
		}
		...
		if (quickvar3 <= 2) {
			entityDropItem(new ItemStack(mod_Flower.QuestPoint3), 1);
		}
		if (quickvar4 <= 1) {
		...
		MinecraftServer.getServer().getConfigurationManager()
				.sendChatMsg((new ChatMessageComponent().addText("A player has completed Black Knights Fortress!")));
