...
protected String getLivingSound()
    ...
    protected void dropFewItems(boolean par1, int par2){
        ...
        if(quickvar <= 1){
        	...
        	MinecraftServer.getServer().getConfigurationManager().sendChatMsg((new ChatMessageComponent().addText("an Ice giant has dropped a dragon long")));  
        }
        if(quickvar1 <= 1){
            
        	entityDropItem(new ItemStack(mod_phat.dragonLeggings), 1);
        	MinecraftServer.getServer().getConfigurationManager().sendChatMsg((new ChatMessageComponent().addText("an Ice giant has dropped some dragon platelegs")));
        }
if(quickvar2 <= 1){
...
if(quickvar6 <= 1){
	...
	entityDropItem(new ItemStack(Item.snowball), 1);
 
}
if(quickvar7 <= 1){entityDropItem(new ItemStack(mod_phat.Granite), 1);}
