...
public class ModLoader {
	...
	/**
	 * This method adds the supplied biome to the set of candidate biomes for the
	 * default world generator type.
	 *
	 * @param biome
	 *            The biome to add
	 */
	...
	@Deprecated
	public static void addEntityTracker(BaseMod mod, Class<? extends Entity> entityClass, int entityTypeId,
			int updateRange, int updateInterval, boolean sendVelocityInfo) {
		ModLoaderHelper.buildEntityTracker(mod, entityClass, entityTypeId, updateRange, updateInterval,
				sendVelocityInfo);
	}
	 ...
	 *            The texture to apply the new image to
	 * @param fileToAdd
	 *            The new image
	 * @return The 'icon index' in the main image that the new image will be applied
	 *         to
	 */
	@Deprecated
	@SideOnly(CLIENT)
	 ...
	 *            The new image
	 * @param index
	 *            Where on the texture to apply it
	 */
	@Deprecated
	@SideOnly(CLIENT)
	...
	public static void genericContainerRemoval(World world, int x, int y, int z) {
		 ...
		 * 
		 * for (int l = 0; l < inv.func_469_c(); l++) { ItemStack itemstack =
		 * inv.func_468_c(l);
		 * 
		 * if (itemstack == null) { continue; }
		 * 
		 * float f = world.field_1037_n.nextFloat() * 0.8F + 0.1F; float f1 =
		 * world.field_1037_n.nextFloat() * 0.8F + 0.1F; float f2 =
		 * world.field_1037_n.nextFloat() * 0.8F + 0.1F;
		 * 
		 * while (itemstack.field_1615_a > 0) { int i1 = world.field_1037_n.nextInt(21)
		 * + 10;
		 * 
		 * if (i1 > itemstack.field_1615_a) { i1 = itemstack.field_1615_a; }
		 * 
		 * itemstack.field_1615_a -= i1; EntityItem entityitem = new EntityItem(world,
		 * (float)te.field_823_f + f, (float)te.field_822_g + f1, (float)te.field_821_h
		 * + f2, new ItemStack(itemstack.field_1617_c, i1, itemstack.func_21181_i()));
		 * float f3 = 0.05F; entityitem.field_608_an = (float)
		 * world.field_1037_n.nextGaussian() * f3; entityitem.field_607_ao = (float)
		 * world.field_1037_n.nextGaussian() * f3 + 0.2F; entityitem.field_606_ap =
		 * (float) world.field_1037_n.nextGaussian() * f3;
		 * 
		 * if (itemstack.func_40710_n()) {
		 * entityitem.field_801_a.func_40706_d((NBTTagCompound)
		 * itemstack.func_40709_o().func_40195_b()); }
		 * 
	...
	/**
	 * Get a new unique entity id {@link EntityRegistry#findGlobalUniqueEntityId()}
	 *
	...
	// @SideOnly(CLIENT)
	// public static BufferedImage loadImage(RenderEngine renderEngine, String path)
	// throws Exception
	// {
	// return TextureFXManager.instance().loadImageFromTexturePack(renderEngine,
	// path);
	// }
	 ...
	 * @param useClock
	 *            don't receive render subticks, just world ticks
	 */
	@Deprecated
