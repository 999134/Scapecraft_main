	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (quickvar1 <= 1) {
			...
			MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
					(new ChatMessageComponent().addText("A White Knight has dropped some dragon platelegs!")));
		}
		if (quickvar2 <= 1) {
	...
	public boolean interact(EntityPlayer par1EntityPlayer) {
		...
		if (props.getTheifcd() >= 99) {
			if (!worldObj.isRemote) {
				ModLoader.getMinecraftInstance().thePlayer.addChatMessage("\u00a7EWhiteKnight: For Falador!");
			}
		}
