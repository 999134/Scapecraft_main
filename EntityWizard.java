...
public class EntityWizard extends EntityMob implements IRangedAttackMob {
	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		if (var11 <= 1) {
			entityDropItem(new ItemStack(mod_BlocksGalore.SaradominHilt), 1);
			MinecraftServer.getServer().getConfigurationManager()
					.sendChatMsg((new ChatMessageComponent().addText("A Wizard has dropped a Saradomin Hilt!")));
		}
		...
		if (props.getTheifcd() >= 99) {
			if (!worldObj.isRemote) {
				ModLoader.getMinecraftInstance().thePlayer
						.addChatMessage("\u00a7EWizard: You assume my services are free..."
